...
function getPinoHttpOption(level: string = 'info'): Options {
  return {
    ...
    redact: {
      paths: ['password', 'req.body.password', 'body.password'],
    },
...
export function getLoggerModuleOptions(configService: ConfigService, prefix: string = ''): Params {
  const app = configService.get('OTLP_SERVICE_NAME') || 'app';
  const level = configService.get(prefix + 'LOG_LEVEL') || 'info';
  const filename = configService.get(prefix + 'LOG_FILE');
  const loki = configService.get(prefix + 'LOG_LOKI');
  console.log('app:', app);
